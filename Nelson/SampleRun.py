...
"System Conditions"
P_evap=445.1*1000 #Evap. pressure [Pa]
P_cond=2221.1*1000 #Cond. pressure [Pa]
P_inj=859.1*1000 #Inj. pressure [Pa]
...
#AS = CP.AbstractState('HEOS', 'R407C') #choose between: 'HEOS','TTSE&HEOS','BICUBIC&HEOS','REFPROP','SRK','PR'
gas  = 'R407C'
from time import time
t1=time()
[W_dot,m_dot_ref,m_dot_inj,T_dis] = CompressorVaporInj_407C.Compressor(gas,T_suc,T_inj,P_evap,P_inj,P_cond,N_c,VR1,VR2,A_suc,A_inj,A_leak,A_dis,V_suc_comp,T_loss,eta_m)

print "Work [W]: ",W_dot
print "Suction massFlow [kg/s]: ",m_dot_ref
print "Injection massFlow [kg/s]: ",m_dot_inj
print "Discharge Temperature [K]: ",T_dis
print 'Took '+str(time()-t1)+' seconds to run Compressor model'
