...
from scipy.optimize import newton,fmin_slsqp, minimize
...
#             self.hout_r+=self.EvapsA[i].mdot_r*self.EvapsA[i].hout_r  #flow weighted average
        ...
        self.T_air_out = 0.0
        for i in range(self.num_evaps):
            self.T_air_out += self.EvapsB[i].Tout_a
        self.T_air_out_avg = self.T_air_out/self.num_evaps
...
def airside_maldistribution_study(evap_type='LRCS',MD_Type=None,interleave_order=None,MD_severity=None,airside_maldistributions=None,num_evaps=2,filenameMDair='debug.csv',Hybrid='equal_flow',adjust_area_fraction_iternum=10):
    #===========================================================================
    # Main Function
    #===========================================================================
    ...
    if MD_Type==None:
        ...
        filenameMDair =evap_type+'-6Circuit_airMD_Ammar_Tuning_Test1_baseline_Tsup=10.csv'
...
def objective(x,evap_type='60K',MD_Type="60K"):
...
#        evap.interleave_order = interleave_order
    ...
    J1 = (((m_r1 - df.m_r[7])/df.m_r[7])**2 + ((m_r2 - df.m_r[6])/df.m_r[6])**2 +
        ((m_r3 - df.m_r[5])/df.m_r[5])**2 + ((m_r4 - df.m_r[4])/df.m_r[4])**2 +
        ((m_r5 - df.m_r[3])/df.m_r[3])**2 + ((m_r6 - df.m_r[2])/df.m_r[2])**2 +
        ((m_rB - df.m_r[1])/df.m_r[1])**2 + ((m_rC - df.m_r[0])/df.m_r[0])**2)
    
    J2 = (((Q1 - df.Q[7])/df.Q[7])**2 + ((Q2 - df.Q[6])/df.Q[6])**2 +
        ((Q3 - df.Q[5])/df.Q[5])**2 + ((Q4 - df.Q[4])/df.Q[4])**2 +
        ((Q5 - df.Q[3])/df.Q[3])**2 + ((Q6 - df.Q[2])/df.Q[2])**2 +
        ((QB - df.Q[1])/df.Q[1])**2 + ((QC - df.Q[0])/df.Q[0])**2)
    
    J3 = (((T_out1 - df.T_a[7])/df.T_a[7])**2 + ((T_out2 - df.T_a[6])/df.T_a[6])**2 +
        ((T_out3 - df.T_a[5])/df.T_a[5])**2 + ((T_out4 - df.T_a[4])/df.T_a[4])**2 +
        ((T_out5 - df.T_a[3])/df.T_a[3])**2 + ((T_out6 - df.T_a[2])/df.T_a[2])**2 +
...
def optimize():
    cons = ()
    bnds = ((0.1, 0.7), (0.1, 0.7))
    guess = (0.2, 0.5) #(h_a, h_tp)
