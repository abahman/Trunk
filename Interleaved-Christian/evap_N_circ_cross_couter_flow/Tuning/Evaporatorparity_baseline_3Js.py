...
ax.plot(mdot_exp/m_mean,mdot_model/m_mean,'o',ms=4,markerfacecolor='None',label='Mass flow rate (MAE = {:0.1f}\%'.format(mape_mass)+', RMSE = {:0.1f}\%)'.format(rmse_mass*100),mec='b',mew=1)
ax.plot(Q_exp/Q_mean,Q_model/Q_mean,'s',ms=4,markerfacecolor='None',label='Cooling capacity (MAE = {:0.1f}\%'.format(mape_Q)+', RMSE = {:0.1f}\%)'.format(rmse_Q*100),mec='r',mew=1)
leg=ax.legend(loc='upper left',numpoints=1)
frame  = leg.get_frame()  
frame.set_linewidth(0.5)
ax.set_xlim((0,ax_max))
