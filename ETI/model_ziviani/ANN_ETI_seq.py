...
#Import compressor map B-PI
from VICompressorLumpkin_single_two_phase import VICompressorLumpkinBPIClass
...
def Calculate():
    ...
    for i in range(0,(end-start+1)):
        ...
        df_final=xl.append(df,ignore_index=True,sort=True)
        df_final.tail()
    ...
    ## Run Domenique B-PI model
    mdot_r_BPI = np.zeros_like(T_env)
    mdot_inj_BPI = np.zeros_like(T_env)
    mdot_tot_BPI = np.zeros_like(T_env)
    Tdis_BPI = np.zeros_like(T_env)
    fq_BPI = np.zeros_like(T_env)
    Wdot_BPI = np.zeros_like(T_env)
    eta_oi_BPI = np.zeros_like(T_env)

    for i in range(len(T_env)):
        ...
        kwds={
            ...
            #'T':[0.8927,1,-0.0295,0.02772,0.2898,0.4556,-0.05758,-0.2855,0.287,0.7001],
            ...
            'AS':AS,
            'Tin_r':T_suc[i],
            'pin_r':P_suc[i]*1000, #Pa
            'pout_r':P_dis[i]*1000, #Pa
            'pinj_r':P_inj[i]*1000, #Pa
            'Tinj_r':T_inj[i],
            'Tamb':T_env[i],
        ...
        Tdis_BPI[i] = Comp.Tout_r
        mdot_r_BPI[i] = Comp.mdot_r
        mdot_inj_BPI[i] = Comp.mdot_inj
        mdot_tot_BPI[i] = Comp.mdot_tot
        Wdot_BPI[i] = Comp.Wdot/1000
        eta_oi_BPI[i] = Comp.eta_oi
        fq_BPI[i] = Comp.fq     
    
    
    #Relative error calculations
    RE_BPI_Tdis = RE(T_dis,Tdis_BPI)*100
    RE_BPI_mdotr = RE(m_suc,mdot_r_BPI)*100
    RE_BPI_mdotinj = RE(m_inj,mdot_inj_BPI)*100
    RE_BPI_Wdot = RE(W_comp,Wdot_BPI)*100

    RE_ANN_Tdis = RE(T_dis,T_dis_ANN)*100
    RE_ANN_mdotr = RE(m_suc,m_suc_ANN)*100
    RE_ANN_mdotinj = RE(m_inj,m_inj_ANN)*100
    RE_ANN_Wdot = RE(W_comp,W_comp_ANN)*100  
    
    print('----- REmean and R^2 of ANN----')
    ...
    print('----- REmean and R^2 of Buckingham-pi----')
    print('T_dis:',REmean(T_dis,Tdis_BPI),Rsquared(T_dis,Tdis_BPI)*100)
    print('m_suc:',REmean(m_suc,mdot_r_BPI),Rsquared(m_suc,mdot_r_BPI)*100)
    print('m_inj:',REmean(m_inj,mdot_inj_BPI),Rsquared(m_inj,mdot_inj_BPI)*100)
    print('m_tot:',REmean(m_tot,mdot_tot_BPI),Rsquared(m_tot,mdot_tot_BPI)*100)
    print('W_comp:',REmean(W_comp,Wdot_BPI),Rsquared(W_comp,Wdot_BPI)*100)
    ...
    plt.plot(T_dis_ANN[:n_training],T_dis[:n_training],'ro',ms = 5,mec='black',mew=0.5,label='Training points')
    plt.plot(T_dis_ANN[-n_split:],T_dis[-n_split:],'b*',ms = 7,mec='black',mew=0.5,label='Testing points')
    
    plt.text(360,350,'R$^2$ = {:0.01f}%\n'.format(Rsquared(T_dis,T_dis_ANN)*100)+'MAE = {:0.01f}%\n'.format(mape(T_dis_ANN,T_dis))+'RMSE = {:0.01f}%\n'.format(rmse(T_dis_ANN,T_dis)),ha='left',va='center',fontsize = 8)
    ...
    x=[Tmin,Tmax]
    y=[Tmin,Tmax]
    y105=[1.01*Tmin,1.01*Tmax]
    y95=[0.99*Tmin,0.99*Tmax]
    ...
    fig.savefig('ANN_T_dis.png',dpi=600)
     
    # Validation m_suc
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(m_suc_ANN[:n_training],m_suc[:n_training],'ro',ms = 5,mec='black',mew=0.5,label='Training points')
    plt.plot(m_suc_ANN[-n_split:],m_suc[-n_split:],'b*',ms = 7,mec='black',mew=0.5,label='Testing points')

    plt.text(0.08,0.06,'R$^2$ = {:0.01f}%\n'.format(Rsquared(m_suc,m_suc_ANN)*100)+'MAE = {:0.01f}%\n'.format(mape(m_suc_ANN,m_suc))+'RMSE = {:0.01f}%\n'.format(rmse(m_suc_ANN,m_suc)),ha='left',va='center',fontsize = 8)
    ...
    fig.savefig('ANN_m_suc.png',dpi=600)
     
    # Validation m_inj
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(m_inj_ANN[:n_training],m_inj[:n_training],'ro',ms = 5,mec='black',mew=0.5,label='Training points')
    plt.plot(m_inj_ANN[-n_split:],m_inj[-n_split:],'b*',ms = 7,mec='black',mew=0.5,label='Testing points')
    plt.text(0.03,0.01,'R$^2$ = {:0.01f}%\n'.format(Rsquared(m_inj,m_inj_ANN)*100)+'MAE = {:0.01f}%\n'.format(mape(m_inj_ANN,m_inj))+'RMSE = {:0.01f}%\n'.format(rmse(m_inj_ANN,m_inj)),ha='left',va='center',fontsize = 8)
    ...
    fig.savefig('ANN_m_inj.png',dpi=600)
     
    # Validation m_tot
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(m_tot_ANN[:n_training],m_tot[:n_training],'ro',ms = 5,mec='black',mew=0.5,label='Training points')
    plt.plot(m_tot_ANN[-n_split:],m_tot[-n_split:],'b*',ms = 7,mec='black',mew=0.5,label='Testing points')
    plt.text(0.10,0.08,'R$^2$ = {:0.01f}%\n'.format(Rsquared(m_tot,m_tot_ANN)*100)+'MAE = {:0.01f}%\n'.format(mape(m_tot_ANN,m_tot))+'RMSE = {:0.01f}%\n'.format(rmse(m_tot_ANN,m_tot)),ha='left',va='center',fontsize = 8)
    ...
    fig.savefig('ANN_m_tot.png',dpi=600)
     
    # Validation W_comp
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(W_comp_ANN[:n_training],W_comp[:n_training],'ro',ms = 5,mec='black',mew=0.5,label='Training points')
    plt.plot(W_comp_ANN[-n_split:],W_comp[-n_split:],'b*',ms = 7,mec='black',mew=0.5,label='Testing points')
    ...
    fig.savefig('ANN_W_comp.png',dpi=600)
...
#     fig.savefig('ANN_f_loss.pdf')
    ...
    plt.plot(T_dis,RE_ANN_Tdis,'ro',ms = 5,mec='black',mew=0.5,label='ANN')
    plt.plot(T_dis,RE_BPI_Tdis,'gs',ms = 5,mec='black',mew=0.5,label='B-$\pi$')
    ...
    plt.tick_params(direction='in')
    plt.show()
    fig.savefig('RE_T_dis.pdf')
    fig.savefig('RE_T_dis.png',dpi=600)
     
    # Validation m_suc
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(m_suc,RE_ANN_mdotr,'ro',ms = 5,mec='black',mew=0.5,label='ANN')
    plt.plot(m_suc,RE_BPI_mdotr,'gs',ms = 5,mec='black',mew=0.5,label='B-$\pi$')

    plt.xlabel('$\dot m_{suc}$ [kg/s]')
    plt.ylabel('RE [$\%$]')
    ...
    plt.tick_params(direction='in')
    plt.show()
    fig.savefig('RE_m_suc.pdf')
    fig.savefig('RE_m_suc.png',dpi=600)
     
    # Validation m_inj
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(m_inj,RE_ANN_mdotinj,'ro',ms = 5,mec='black',mew=0.5,label='ANN')
    plt.plot(m_inj,RE_BPI_mdotinj,'gs',ms = 5,mec='black',mew=0.5,label='B-$\pi$')

    plt.xlabel('$\dot m_{inj}$ [kg/s]')
    plt.ylabel('RE [$\%$]')
    ...
    plt.tick_params(direction='in')
    plt.show()
    fig.savefig('RE_m_inj.pdf')
    fig.savefig('RE_m_inj.png',dpi=600)
     

    # Validation W_comp
    fig=pylab.figure(figsize=(4,4))
 
    plt.plot(W_comp,RE_ANN_Wdot,'ro',ms = 5,mec='black',mew=0.5,label='ANN')
    plt.plot(W_comp,RE_BPI_Wdot,'gs',ms = 5,mec='black',mew=0.5,label='B-$\pi$')
 
    plt.xlabel('$\dot W_{comp}$ [kW]')
    plt.ylabel('RE [$\%$]')
    ...
    plt.tick_params(direction='in')
    plt.show()
    fig.savefig('RE_W_comp.pdf')
    fig.savefig('RE_W_comp.png',dpi=600)
